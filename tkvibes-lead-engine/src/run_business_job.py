...
3. Push leads to CRM (assigned: India→Jashmit Bhalla, Canada→Tishya Kane)
4. Generate sample websites + pitch decks (with competitor research
   for no-website leads, GTmetrix/performance analysis for weak sites)
5. Push HTML copies to GitHub under "Sample Webpages and pitch deck"
...
Usage:
    ...
    python -m src.run_business_job --dry-run          # discover+score+generate only (no sheet/CRM/github)
"""
...
def main():
    ...
    print("   Leads → Sheets → CRM (assigned) → sample sites + pitch decks → GitHub → CRM downloads")
